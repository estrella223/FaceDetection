...
from haar_utils import *  # 분류기 적재 및 전처리 함수
from haar_histogram import *  #히스토그램 비교 관련 함수

face_cascade = cv2.CascadeClassifier("haarcascade_frontalface_alt2.xml") # 얼굴 분류기
eye_cascade = cv2.CascadeClassifier("haarcascade_eye.xml") # 눈 분류기
image, gray = preprocessing(34) # 전처리
if image is None: raise Exception("영상파일 읽기 에러")

faces = face_cascade.detectMultiScale(gray, 1.1, 10, 0, (100, 100)) # 얼굴 검출
if faces.any():
    x, y, w, h = faces[0]
    ...
    if len(eyes) == 2:
        ...
        print("입술-얼굴 유사도: %4.2f" % sim[0])  # 결과출력
